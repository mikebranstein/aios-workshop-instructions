...
from aios_orchestration_core.metrics import InstrumentedLLMAdapter, MetricsService
...
def _apply_wiki_manager_plan(
    ...
    """Invoke the wiki_manager LLM and write the wiki page. Returns the written page
    path on success, or empty string if the write fails."""
    ...
    try:
        ...
        logger.warning(
            f"    Research #{research_issue_number}: wiki write failed — {exc}"
        )
        return ""


def _next_adr_number(gateway) -> int:
    """Return the next available 0-based ADR sequence number from docs/adr/."""
    existing = gateway.list_adr_files()
    if not existing:
        return 0
    numbers = []
    for path in existing:
        stem = Path(path).stem  # e.g. "0001-choose-postgres"
...
def _assemble_adr_content(
    *,
    ...
    """Assemble full ADR markdown from adr_generator LLM output sections."""
    ...
    sections = [
        ...
        payload.get("decision_section", "## Decision\n\nNo decision provided."),
        payload.get("alternatives_section", "## Alternatives Considered\n\nNone documented."),
        payload.get("rationale_section", "## Rationale\n\nNo rationale provided."),
        payload.get("consequences_section", "## Consequences\n\nNone documented."),
        payload.get("validation_strategy_section", "## Validation Strategy\n\nNone defined."),
...
def _process_research_issue(
    ...
    span_key = f"worker:{foundation_issue_number}:{linked_issue.number}"
    with metrics.span(span_key, type="worker", issue_number=foundation_issue_number, worker_issue=linked_issue.number):
        return _process_research_issue_inner(
            gateway, adapter, foundation_issue_number, foundation_markdown, linked_issue
        )


def _process_research_issue_inner(
    gateway,
    adapter: JudgmentLLMAdapter,
    foundation_issue_number: int,
    foundation_markdown: str,
    ...
    """Core logic for processing a single research issue (called inside a metrics span)."""
    ...
    if decision == "COMPLETE":
        # Always write wiki for COMPLETE decisions; use summary as fallback content.
        wiki_page_path = _apply_wiki_manager_plan(
            ...
            summary=summary,
            wiki_title=wiki_title,
            wiki_summary=wiki_summary or summary,
            adr_link=adr_link,
        )
        # Generate and write the ADR file to docs/adr/ when a title is available.
        if adr_title:
            adr_link = _generate_and_write_adr(
                gateway=gateway,
                adapter=adapter,
                foundation_issue_number=foundation_issue_number,
                research_issue_number=linked_issue.number,
        ...
        # issue. The gate later requires both wiki and ADR links in the parent
        # issue's comments; closing without them creates an unresolvable deadlock.
        closure_failures: list[str] = []
        if not wiki_page_path:
            closure_failures.append("wiki page write returned no path")
        if not adr_title:
            closure_failures.append("adr_title missing — ADR is required for closure")
        elif not adr_link:
            closure_failures.append(f"ADR file write failed for title: {adr_title!r}")

        if closure_failures:
            failure_detail = "; ".join(closure_failures)
            logger.warning(
                f"    Research #{linked_issue.number}: closure prerequisites not met "
                f"({failure_detail}); leaving open for retry"
            )
            # Do NOT post the "Foundation research worker decision:" marker — omitting
            # it lets the next pass re-invoke the LLM and try again, feeding into the
            # existing 2-retry-then-BLOCKED safety valve.
            failure_comment = (
                f"Foundation research closure blocked: artifacts not written.\n\n"
                f"Failures: {failure_detail}\n\n"
                "This issue will be retried on the next orchestrator pass."
            )
            gateway.post_comment(linked_issue.number, failure_comment)
            return {
                "issue_number": linked_issue.number,
                "decision": "NEEDS_MORE_RESEARCH",
                "summary": f"Closure blocked — {failure_detail}",
                "worker_comment": None,
                "primary_comment": None,
                "blocked": False,
                "completed": False,
...
def _run_linked_research_workers(
    ...
    worker_span_keys = [f"worker:{foundation_issue_number}:{li.number}" for li in linked_issues]
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Log per-worker summary now that the span is closed
                _metrics.log_summary(
                    f"Worker #{issue_number} (parent #{foundation_issue_number})",
                    span_keys=[f"worker:{foundation_issue_number}:{issue_number}"],
                    include_call_breakdown=True,
                )
    ...
    # Worker-stage rollup across all workers for this issue
    _metrics.log_summary(
        f"Issue #{foundation_issue_number} — worker stage total",
        span_keys=worker_span_keys,
    )
    return {"completed": completed, "touched": touched, "blocked": blocked}
...
def _process_foundation_pass(
    ...
    for issue, current_state in actionable:
        issue_number = issue.number
        issue_span_key = f"issue:{issue_number}:pass:{pass_index}"
        issue_span_keys.append(issue_span_key)
...
def main():
    ...
    try:
        ...
        raw_adapter = create_adapter(model=args.model, use_stub=args.stub, stub_class=StubLLMAdapter)
        metrics = MetricsService()
