...
## Cycle: pipeline routing (v2 - intake + design, depth-first)

**Depth-first approach:** Find the FIRST issue that has not been completed or blocked, and advance it one stage further. Then on the next cycle, find the next issue. This ensures each issue flows through intake and design before starting a new one.
...
| design-blocked                    | Skip to next issue. Needs human revision before re-evaluation.  |
...
3. Iterate through issues. For the FIRST issue that is not blocked and not design-approved:
...
5. Output cycle summary:
   ...
   echo "Issues awaiting next stage: X"
